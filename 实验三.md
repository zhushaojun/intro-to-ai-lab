...
def main():
    ...
    print(f"   样例批次形状：images={tuple(sample_images.shape)} labels={tuple(sample_labels.shape)} (B,1,28,28 / (B,))")
    ...
    if args.optimizer == "adam":
        ...
        print("   使用 Adam 优化器（对学习率较不敏感，入门更稳）")
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9)
        print("   使用 SGD 优化器（可配合较大学习率与动量）")
